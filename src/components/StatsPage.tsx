...
export const StatsPage: React.FC = () => {
  ...
  const streak = cardQuery.getStreak();
  const daily = cardQuery.getDailyCounts(7);
  const categories = cardQuery.getCategoryCounts();
  const ratings = cardQuery.getRatingCounts();
  const recent = cardQuery.getRecentLogs(20);
  ...
  return (
    <>
        ...
        {daily.map((d, i) => (
          <div key={i} className="chart-bar-col">
            ...
            <div className="chart-bar-wrapper">
              <div className={`chart-bar ${i === 6 ? 'today' : ''}`} style={{ height: `${(d.count / maxDaily) * 100}%` }} />
            </div>
      ...
      {recent.length === 0 ? (
        <div className="empty-state" style={{ padding: 20 }}><div>No review history yet</div></div>
      ) : (
    ...
    <>
      ...
      <div className="card-list">
        <div className="distribution-row">
          <span className="dist-label">Retention</span>
          <span className="dist-value">{(p.request_retention * 100).toFixed(0)}%</span>
        </div>
        <div className="distribution-row">
          <span className="dist-label">Max Interval</span>
          <span className="dist-value">{p.maximum_interval} days</span>
        </div>
        <div className="distribution-row">
          <span className="dist-label">Fuzz</span>
          <span className="dist-value">{p.enable_fuzz ? 'On' : 'Off'}</span>
        </div>
        <div className="distribution-row">
          <span className="dist-label">Short Term</span>
          <span className="dist-value">{p.enable_short_term ? 'On' : 'Off'}</span>
        </div>
        <div className="distribution-row">
          <span className="dist-label">Learning Steps</span>
          <span className="dist-value" style={{ fontFamily: 'var(--font-mono)', fontSize: '0.7rem' }}>{p.learning_steps.join(', ')}</span>
        </div>
        <div className="distribution-row">
          <span className="dist-label">Relearning Steps</span>
          <span className="dist-value" style={{ fontFamily: 'var(--font-mono)', fontSize: '0.7rem' }}>{p.relearning_steps.join(', ')}</span>
        </div>
        <div className="distribution-row">
          <span className="dist-label">Weights (w)</span>
          <span className="dist-value" style={{ fontFamily: 'var(--font-mono)', fontSize: '0.62rem' }}>{p.w.map(v => v.toFixed(4)).join(', ')}</span>
        </div>
      </div>
    </>
  );
