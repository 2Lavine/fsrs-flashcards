...
export const ReviewPage: React.FC = () => {
  ...
  useReviewHotkeys([
    { keys: '1', enabled: () => !!s.getCard() && s.getRevealed(), handler: () => s.rate(1 as Grade) },
    { keys: '2', enabled: () => !!s.getCard() && s.getRevealed(), handler: () => s.rate(2 as Grade) },
    { keys: '3', enabled: () => !!s.getCard() && s.getRevealed(), handler: () => s.rate(3 as Grade) },
    { keys: '4', enabled: () => !!s.getCard() && s.getRevealed(), handler: () => s.rate(4 as Grade) },
    { keys: 'mod+z', enabled: () => s.getHistoryLength() > 0, handler: () => s.undo() },
    { keys: 'a', enabled: () => s.getHistoryLength() > 0, handler: () => s.undo() },
    { keys: 'space', enabled: () => !!s.getCard() && !s.getRevealed(), handler: () => s.reveal() },
    { keys: 'd', enabled: () => !!s.getCard(), handler: () => s.deleteCurrentCard() },
  ]);
        ...
        <>
          ...
          <div className="flex justify-between text-center">
            {[
              ...
              <div key={st.lbl} className="flex flex-col items-center gap-0.5">
                <span className={`text-xl font-bold tabular-nums ${st.cls ?? ''}`}>{st.num}</span>
          ...
          {s.decks.length > 1 && (
            <div className="flex flex-wrap gap-1.5">
              <Badge variant={s.deckId === '' ? 'default' : 'outline'} className="cursor-pointer" onClick={() => s.setDeckId('')}>All Decks</Badge>
              {s.decks.map(d => (
                <Badge key={d.id} variant={s.deckId === d.id ? 'default' : 'outline'} className="cursor-pointer" onClick={() => s.setDeckId(d.id)}>{d.name}</Badge>
              ))}
            </div>
          )}
          ...
          {s.categories.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              <Badge variant={s.category === '' ? 'default' : 'outline'} className="cursor-pointer" onClick={() => s.setCategory('')}>All</Badge>
              {(s.showAllCats ? s.categories : s.categories.slice(0, 8)).map(c => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="ml-1.5 text-xs opacity-60 hover:opacity-100" onClick={e => { e.stopPropagation(); s.togglePauseCategory(c); }}>
                      {paused ? '▶' : '⏸'}
                    </span>
                  </Badge>
                );
              ...
              {s.categories.length > 8 && (
                <Badge variant="outline" className="cursor-pointer" onClick={() => s.setShowAllCats(!s.showAllCats)}>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-wrap gap-2">
              {s.historyLength > 0 && <Button variant="outline" size="sm" onClick={s.undo}>Undo</Button>}
              {cardPresets.map((p, i) => (
              ...
              <Button variant="ghost" size="sm" className="text-destructive hover:text-destructive" onClick={s.deleteCurrentCard}>Delete</Button>
            </div>
            ...
            {s.deleting ? (
              ...
              <>
                ...
                >
                  {!s.revealed && <div className="text-xs text-muted-foreground absolute top-3 left-1/2 -translate-x-1/2">Tap or Space to reveal</div>}
                  {s.card.deck && <span className="absolute top-3 right-4 text-xs text-muted-foreground">{s.card.deck}</span>}
                  {s.card.category && <span className="absolute top-3 left-4 text-xs text-muted-foreground">{s.card.category}</span>}
                  <div className="text-lg leading-relaxed" dangerouslySetInnerHTML={{ __html: renderCloze(s.card.question, s.revealed) }} />
                  {s.revealed && (
                    <div className="mt-6 pt-6 border-t text-base leading-relaxed text-muted-foreground" dangerouslySetInnerHTML={{ __html: renderCloze(s.card.answer, true) }} />
                  )}
                </div>
                ...
                {s.revealed && s.previewCache && (
                  <div className="grid grid-cols-4 gap-2">
                    {[
                      { key: Rating.Again, cls: 'again', label: ratingLabel(1), hint: '1', desc: formatDate(s.previewCache[1]?.card.due), style: ratingStyles[Rating.Again] },
                      { key: Rating.Hard, cls: 'hard', label: ratingLabel(2), hint: '2', desc: formatDate(s.previewCache[2]?.card.due), style: ratingStyles[Rating.Hard] },
                      { key: Rating.Good, cls: 'good', label: ratingLabel(3), hint: '3', desc: formatDate(s.previewCache[3]?.card.due), style: ratingStyles[Rating.Good] },
                      { key: Rating.Easy, cls: 'easy', label: ratingLabel(4), hint: '4', desc: formatDate(s.previewCache[4]?.card.due), style: ratingStyles[Rating.Easy] },
                    ].map(r => (
                      <Button
                        key={r.key}
                        variant="outline"
