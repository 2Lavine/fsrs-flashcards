...
export const StatsPage: React.FC = () => {
  ...
  const [decks, setDecks] = useState<
    { name: string; cardCount: number; reviewCount: number }[]
  >([]);
  const [ratings, setRatings] = useState<{ label: string; count: number }[]>(
    [],
  );
  const [recent, setRecent] = useState<
    { id: number; rating: number; due: Date; review: Date; question: string }[]
  >([]);
  ...
  if (daily.length > 0) {
    ...
    while (padded.length % 7 !== 0)
      padded.push({ label: "", date: "", count: -1 });
        ...
        <div className="flex gap-0.5">
          ...
          <div className="flex flex-col gap-0.5 mr-2">
            {DAY_LABELS.map((l, i) => (
              <div
                key={i}
                className="h-4 text-xs text-muted-foreground leading-4"
              >
                {l}
              </div>
            ))}
              ...
              return (
                <div key={d.name} className="flex items-center gap-2">
                  <span className="text-sm w-28 shrink-0 truncate">
                    {d.name || "(uncategorized)"}
                  </span>
                  <div className="flex-1 h-2 bg-muted rounded-full overflow-hidden">
                    <div
                      className={`h-full rounded-full ${barColors[i % barColors.length]}`}
                      style={{
                        width: `${stats.total > 0 ? (d.cardCount / stats.total) * 100 : 0}%`,
                      }}
                    />
                  </div>
              ...
              <div key={d.label} className="flex items-center gap-2">
                ...
                <div className="flex-1 h-2 bg-muted rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: `${(d.count / totalRat) * 100}%` }}
                  />
                </div>
            ...
            {recent.map((l) => (
              ...
              >
                ...
                <span className="text-xs text-muted-foreground">
                  {new Date(l.review).toLocaleString()}
                </span>
              </div>
            ))}
          </div>
      ...
      <div>
        ...
        <div className="flex flex-col gap-1.5">
          <div className="flex justify-between text-sm">
            <span>Retention</span>
            <span>{(p.request_retention * 100).toFixed(0)}%</span>
          </div>
          <div className="flex justify-between text-sm">
            <span>Max Interval</span>
            <span>{p.maximum_interval} days</span>
          </div>
          <div className="flex justify-between text-sm">
            <span>Fuzz</span>
            <span>{p.enable_fuzz ? "On" : "Off"}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span>Short Term</span>
            <span>{p.enable_short_term ? "On" : "Off"}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span>Learning Steps</span>
            <span className="font-mono text-xs">
              {p.learning_steps.join(", ")}
            </span>
          </div>
          <div className="flex justify-between text-sm">
            <span>Relearning Steps</span>
            <span className="font-mono text-xs">
              {p.relearning_steps.join(", ")}
            </span>
          </div>
        </div>
      </div>
